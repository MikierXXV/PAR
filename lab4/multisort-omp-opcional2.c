...
/*
long min(long length, long c){
    if (length < c) return length;
    return c;
}
*/
 /*volatile long i,j;
    ...
    // inicializar cada primer elemento de cada chunk
    // inicializar los siguientes
    for (i = 0; i < length; i+= chunk_size) {
        printf("%ld \n", i);
        #pragma omp task firstprivate(i)
        for (j = i; i < min(length, i+chunk_size); j++){
            if (!(j%chunk_size)) {
                data[j] = rand();
            } else {
                data[j] = ((data[j-1]+1) * j * 104723L) % N;
            }
        }
    }*/
